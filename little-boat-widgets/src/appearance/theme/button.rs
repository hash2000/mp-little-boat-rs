...
fn button(foreground: Color, background: Color, background_hover: Color, status: Status) -> Style {
  match status {
    ...
    Status::Disabled => {
      ...
      Style { text_color: Color { a: 0.2, ..active.text_color }, ..active }
    }
  }
}
...
pub fn primary(theme: &Theme, status: Status, selected: bool) -> Style {
  ...
  let background =
    if selected { button_colors.background_selected } else { button_colors.background };

  let background_hover =
    if selected { button_colors.background_selected_hover } else { button_colors.background_hover };
...
pub fn bare(_theme: &Theme, status: Status) -> Style {
  match status {
    Status::Active | Status::Pressed | Status::Hovered => {
      Style { background: Some(Background::Color(Color::TRANSPARENT)), ..Default::default() }
    }
    Status::Disabled => {
      let active = bare(_theme, Status::Active);

      Style {
        text_color: Color { a: 0.2, ..active.text_color },
        border: Border { color: Color { a: 0.2, ..active.text_color }, ..Default::default() },
        ..active
      }
    }
  }
...
pub fn preview_card(theme: &Theme, status: Status) -> Style {
  ...
  let border = Border { radius: 4.0.into(), width: 1.0, color: theme.styles().general.border };
