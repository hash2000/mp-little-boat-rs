...
use iced::{Element, Event, Length, Point, Rectangle, Size, Task, Vector, mouse};
...
impl State {
  pub fn new() -> Self {
    State { status: Status::Closed(false), menu_tree: widget::Tree::empty() }
  }
}
  ...
  Renderer: advanced::Renderer + 'a,
{
  ...
  fn state(&self) -> tree::State {
    tree::State::new(State { status: Status::Closed(false), menu_tree: widget::Tree::empty() })
  }
  ...
  fn update(
    ...
    if is_mouse_event {
      ...
      let is_activation_mouse_event =
        *event == Event::Mouse(mouse::Event::ButtonPressed(self.activation_button));

      let position = if is_activation_mouse_event {
        match self.anchor {
          Anchor::Widget => cursor.position_over(layout.bounds()).map(|_| {
      ...
      let next_status =
        match (is_activation_mouse_event, prev_status, self.toggle_behavior, position) {
          (true, _, ToggleBehavior::KeepOpen, Some(position))
          | (true, Status::Closed(false), ToggleBehavior::Close, Some(position)) => {
      ...
      if next_status != prev_status {
        ...
        if !matches!(next_status, Status::Closed(_)) || !matches!(prev_status, Status::Closed(_)) {
  ...
  fn overlay<'b>(
    ...
    let base =
      self.base.as_widget_mut().overlay(base_state, layout, renderer, viewport, translation);

    let state = tree.state.downcast_mut::<State>();

    let overlay = overlay(state, &mut self.menu, &self.entries, &self.entry, translation);
  ...
  Renderer: advanced::Renderer + 'a,
{
  let build_menu =
    |length, view: &(dyn Fn(T, Length) -> Element<'a, Message, Theme, Renderer> + 'a)| {
      container(column(entries.iter().copied().map(|entry| view(entry, length))))
        .padding(4)
  ...
  Renderer: advanced::Renderer + 'a,
{
  ...
  state.status.open().zip(menu.as_mut()).map(|(position, menu)| {
    overlay::Element::new(Box::new(Overlay { menu, state, position: position + translation }))
  })
}
...
pub fn close<Message: 'static + Send>(f: fn(bool) -> Message) -> Task<Message> {
  ...
  widget::operate(Close { any_closed: false, f })
}
  ...
  Renderer: advanced::Renderer,
{
  fn layout(&mut self, renderer: &Renderer, bounds: Size) -> layout::Node {
    let limits = layout::Limits::new(Size::ZERO, bounds).width(Length::Fill).height(Length::Fill);

    let node = self.menu.as_widget_mut().layout(&mut self.state.menu_tree, renderer, &limits);
  ...
  fn operate(
    ...
    self.menu.as_widget_mut().operate(&mut self.state.menu_tree, layout, renderer, operation);
  }
