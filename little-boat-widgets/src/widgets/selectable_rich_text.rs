...
use super::selectable_text::{Catalog, Interaction, Style, StyleFn};
    ...
    Renderer: text::Renderer + 'a,
{
        ...
        match event {
            ...
            | iced::Event::Touch(touch::Event::FingerPressed { .. }) => {
                ...
                if let Some(cursor) = cursor.position() {
                    state.interaction =
                        Interaction::Selecting(selectable_text::Raw {
                            start: cursor,
                            end: cursor,
                        });
                } else {
    ...
    fn operate(
        ...
        if let Some(selection) = state
            .interaction
            .selection()
            .and_then(|raw| selectable_text::selection(raw, bounds, &state.paragraph, &value))
        {
            let mut content =
