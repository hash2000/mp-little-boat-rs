...
macro_rules! default {
  ($name:ident, $k:tt) => {
    pub fn $name() -> KeyBind {
      KeyBind {
        key_code: KeyCode(iced_core::keyboard::Key::Named(iced_core::keyboard::key::Named::$k)),
  ...
  ($name:ident, $k:literal, $m:expr) => {
    pub fn $name() -> KeyBind {
      KeyBind { key_code: KeyCode(iced_core::keyboard::Key::Character($k.into())), modifiers: $m }
    }
  };
...
impl KeyBind {
  ...
  pub fn is_pressed(&self, key_code: impl Into<KeyCode>, modifiers: impl Into<Modifiers>) -> bool {
...
impl From<(keyboard::Key, keyboard::Modifiers)> for KeyBind {
  fn from((key_code, modifiers): (keyboard::Key, keyboard::Modifiers)) -> Self {
    Self { key_code: KeyCode(key_code), modifiers: Modifiers(modifiers) }
  }
}
...
impl<'de> Deserialize<'de> for KeyBind {
    ...
    D: serde::Deserializer<'de>,
  {
    ...
    let (key_code, modifiers) = match parts.len() {
      ...
      1 => (parts[0].parse::<KeyCode>().map_err(de::Error::custom)?, Modifiers::default()),
      _ => {
        ...
        let key_code = parts[parts.len() - 1].parse::<KeyCode>().map_err(de::Error::custom)?;
        (key_code, modifiers)
      }
    };

    Ok(KeyBind { key_code, modifiers })
  }
}
...
impl fmt::Display for Modifiers {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    if mods.is_empty() { write!(f, "") } else { write!(f, "{}", mods.join(" ")) }
  }
}
