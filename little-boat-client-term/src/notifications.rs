...
impl Notification {
  pub fn new(message: String, level: NotificationLevel) -> Self {
    Self { message, level, timestamp: Instant::now() }
  }
...
impl NotificationManager {
  pub fn new(max_notifications: usize, display_duration: Duration) -> Self {
    Self { notifications: Vec::new(), max_notifications, display_duration }
  }
  ...
  pub fn get_visible_notifications(&self) -> Vec<&Notification> {
    self.notifications.iter().filter(|n| !n.is_expired(self.display_duration)).collect()
  }
